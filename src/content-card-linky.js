        ...
        return html` <div class="main-info">
          ...
          <div class="hp-hc-block">
            ...
            ><span class="conso-unit-hc">
              ${attributes.unit_of_measurement}
              <span class="more-unit">${localize(this.hass, "card.in_off_peak")}</span></span
            ><br />
            <span class="conso-hp">${this.toFloat(attributes.yesterday_HP)}</span
            ><span class="conso-unit-hp">
              ${attributes.unit_of_measurement}
              <span class="more-unit">${localize(this.hass, "card.in_peak")}</span></span
            >
          </div>
  ...
  renderPrice(attributes) {
    if (this.config.showPrice) {
      return html` <div class="cout-block">
        <span class="cout" title="${localize(this.hass, "card.daily_cost")}"
          >${this.toFloat(attributes.daily_cost, 2)}</span
      ...
      if (attributes.serviceEnedis !== "myElectricalData") {
        return html`
          <div class="information-msg" style="color: var(--error-color, red)">
            <ha-icon id="icon" icon="mdi:alert-outline"></ha-icon>
            ${localize(this.hass, "card.migrate_med")}
          </div>
        `;
      }
    }
  ...
  renderVersion(versionUpdateAvailable, versionGit) {
    if (versionUpdateAvailable === true) {
      return html`
        <div class="information-msg" style="color: var(--error-color, red)">
          <ha-icon id="icon" icon="mdi:alert-outline"></ha-icon>
          ${localize(this.hass, "card.version_available", { version: versionGit })}
        </div>
      `;
    } else {
      return html``;
    ...
    return html`
      <div class="week-summary-card" style="background: ${dynamicGradient}">
        <div class="week-summary-header">
          ...
          <span class="week-summary-title">${localize(this.hass, "card.current_week")}</span>
          <span class="week-summary-period"
            >${localize(this.hass, "card.since")}
            ${mondayThisWeek.toLocaleDateString(this.hass?.locale?.language || "fr-FR", {
              day: "numeric",
              month: "short",
            })}</span
          >
